...
class RotaryEmbedding(nn.Module):
    ...
    def __init__(
        ...
        # PATCH (AIDO_RAGPLM): upstream stored inv_freq as a plain attribute on the
        # current CUDA device, so `model.to('mps')` (or any non-CUDA device) left
        # it on CPU and the rotary indexing later crashed with a device mismatch.
        # Register it as a non-persistent buffer instead so it follows `.to()`.
        self.register_buffer(
            "inv_freq",
            1.0
            / (
